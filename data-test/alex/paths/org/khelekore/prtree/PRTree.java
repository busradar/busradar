...
public class PRTree<T> {
	...
	public void write(DataOutputStream s) throws IOException {
    	s.writeInt(branchFactor);
    	
    	if (root instanceof LeafNode) {
    		s.writeBoolean(true);
    		((LeafNode)root).write(s);
    	}
    	else if (root instanceof InternalNode) {
    		s.writeBoolean(false);
    		((InternalNode)root).write(s);
    	}
    	else
    ...
    @SuppressWarnings("unchecked")
	public PRTree(DataInputStream s) throws IOException {
    	converter = (MBRConverter<T>) new RouteTree.RouteMBRConverter();
    	branchFactor = s.readInt();
    	
    	if (s.readBoolean() == true) {
    		root = new LeafNode(s);
    	}
    	else {
    		root = new InternalNode(s);
    	}
