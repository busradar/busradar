...
public class Stop
{
	...
	public Stop(int id, double lt, double ln, int d, String n)
	{
		super();
		lat = lt;
		lon = ln;
	...
	public static Stop[] getAllStops()
	{
		if (allStops == null)
		{
			...
			{
				allStops[x] = new Stop(select.getInt(0), select.getDouble(1), select.getDouble(2), select.getInt(3), select.getString(4));
				select.moveToNext();
			}
			select.close();
		}
